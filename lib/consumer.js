...
module.exports = class Consumer {
  ...
  constructor (opts) {
    ...
    process.on('SIGTERM', this.__disposeBinded);
  }
      ...
      this.kafka.consumer(opts.consumerGroup)
        .join(consumerOpts, (err, consumerInstance) => {
          if (err) {
            ...
            this.log.error('kafka.consumer.connect() :: Failed to connect. Retry:',
              this.connectRetries);
  ...
  _onError (err) {
    if (err.data && err.data['error_code'] === 40401) {
      ...
      this.log.info('kafka.consumer._onError() :: Detected "topic not found" error',
        'resetting consumer...');
  ...
  dispose () {
    return new Promise((resolve) => {
      ...
      this.log.info('kafka.consumer.dispose() :: shutting down consumer...');
